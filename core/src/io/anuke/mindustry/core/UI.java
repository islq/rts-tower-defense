...
public class UI extends SceneModule{
    ...
    // delay when recover ui
    private float disableDelay = 0f;
    public void disable(boolean disable) {
        if (Vars.disableUI == disable) return;
        Vars.disableUI = disable;
        Log.err("Vars.disableUI set to " + disable);
        if (disable) {
            Inputs.removeProcessor(scene);
        } else {
    ...
    public boolean hasMouse() {
        if (Vars.disableUI) return false;
    ...
    public boolean hasMouse(float mousex, float mousey){
        if (Vars.disableUI) return false;
        return super.hasMouse(mousex, mousey);
    }
    ...
    public void update(){
        ...
        if (disableDelay > 0) {
            disableDelay -= Gdx.graphics.getDeltaTime();
            return;
        }
