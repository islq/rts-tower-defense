    ...
    public void drawTop(){
        ...
        for(Player player : players){
            ...
            if(input.recipe == null && !ui.hasMouse()){
                ...
                if (tile == null) {
                    if (players[0].target != null) {
                        tile = world.tileWorld(players[0].target.getX(), players[0].target.getY());
                    }
                }

                if(tile != null && tile.block() != Blocks.air){
                    Tile target = tile.target();
                    boolean sameTeam = tile.target().getTeam() == players[0].getTeam();
                    ...
                    if(entity != null){
                        ...
                        Runnable drawbars = () -> {
                            for(BlockBar bar : block.bars.list()){
                                if (!sameTeam && bar.type != BarType.health) continue;
