...
public class MobileInput extends InputHandler implements GestureListener{
    ...
    public void removeRequests(int tileX1, int tileY1, int tileX2, int tileY2) {
        Array<PlaceRequest> badArray = new Array<PlaceRequest>();
        float xMin = Math.min(tileX1, tileX2);
        float xMax = Math.max(tileX1, tileX2);
        float yMin = Math.min(tileY1, tileY2);
        float yMax = Math.max(tileY1, tileY2);
        for(PlaceRequest req : selection){
            Tile other = req.tile();
        ...
        for(PlaceRequest req : badArray) {
            selection.removeValue(req, false);
        }
    }

    public boolean hasPlaced(int tileX, int tileY, Block block) {
        PlaceRequest request = null;
        Tile tile = world.tile(tileX, tileY);
        if (tile != null) {
            request = getRequest(tile);
        }

        if (request != null && !request.remove && request.recipe.result == block) {
            return true;
        }
        return false;
    }

    public void tryPlaceAll(int x, int y) {
        if (!lineMode) return;
        touchUp(x, y, 0, 1);
    }

    public boolean isBadLineMode(int tileX, int tileY) {
        if(lineMode) {
...
//        }
    ...
    Element btnCancel;
    @Override
    public Element getUIElement(String name) {
    ...
    public void buildUI(Table table){
        ...
        btnBreak = table.addImageButton("icon-break", "clear-toggle-partial", 16 * 2f, () -> {
            mode = mode == breaking ? recipe == null ? none : placing : breaking;
            lastRecipe = recipe;
            if(mode == breaking){
                showGuide("deconstruction");
            }
        }).update(l -> l.setChecked(mode == breaking)).getElement();

        //rotate button
        btnRotate = table.addImageButton("icon-arrow", "clear-partial", 16 * 2f, () -> {
            rotation = Mathf.mod(rotation + 1, 4);
            if (mode == placing && selection.size > 0 && !selection.peek().remove) {
                selection.peek().rotation = rotation;
            }
        })
        .update(i -> {
            if (i.getImage().getOriginX() == 0) i.getImage().setRotation(0);
            i.getImage().setRotationOrigin(rotation * 90, Align.center);
        })
        .visible(() -> recipe != null && recipe.result.rotate).getElement();
        ((ImageButton)btnRotate).getImage().setOrigin(Align.center);

        //cancel button
        btnCancel = table.addImageButton("icon-cancel", "clear-partial", 16 * 2f, () -> {
            selecting = false;
            if (mode != none && selection.size > 0) {
                selection.clear();
            } else {
        ...
        //confirm button
        btnCheck = table.addImageButton("icon-check", "clear-partial", 16 * 2f, () -> {
            ...
            selection.clear();
            selecting = false;
        }).visible(() -> !selection.isEmpty()).getElement();
    }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button){
        // Log.err("touch x:" + screenX + " y:" + screenY);
    ...
    public boolean longPress(float x, float y){
        ...
        if (!Vars.guide.isPlaceEnable(cursor)) return false;
    ...
    public boolean tap(float x, float y, int count, int button){
        if (Vars.disableTap) return false;
        ...
        //ignore off-screen taps
        if(cursor == null || !Vars.disableUI && ui.hasMouse(x, y)) return false;

        // Log.err("tap:" + cursor.x + "," + cursor.y);
        ...
        if(hasRequest(cursor)){
            ...
            if (Vars.guide.isPlaceEnable(cursor)) {
    ...
    public void update(){
        ...
        if(lineMode){
            ...
            if(Gdx.input.isTouched(0) && lineMode && Vars.guide.isPanEnable()){
    ...
    public boolean pan(float x, float y, float deltaX, float deltaY){
        if (!Vars.guide.isPanEnable()) return false;
