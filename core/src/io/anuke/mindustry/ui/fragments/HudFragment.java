...
public class HudFragment extends Fragment{
    ...
    public void build(Group parent){
        ...
        parent.fill(cont -> {
            ...
            cont.top().left().visible(() -> !state.is(State.menu) && !Vars.guide.noMenu());
            ...
            if(mobile){
                cont.table(select -> {
                    ...
                    select.visible(() -> !Vars.guide.inGuideMode());
                });
        ...
        parent.fill(t -> t.top().right().add(new Minimap())
            .visible(() -> !state.is(State.menu) && !Vars.guide.noMiniMap() && Settings.getBool("minimap")));
        ...
        parent.fill(t-> {
           t.table(tt -> blockfrag.build(tt)).grow();
           t.row();
           Table line = new Table();
           line.background("scroll");
           line.left().visible(() -> !state.is(State.menu) && shown);;
           int nItem = content.items().size;
           int total = 0;
           line.addImage("heart").size(8*2).padLeft(5f);
           line.label(() -> {
               Tile core = state.getCore();
               if (core == null) {
                   return "0";
               } else {
                   return ui.formatAmount((int) core.entity.health);
               }
           }).padLeft(2f);
            total ++;
           for(int i=0; i<nItem; i++) {
               Item item = content.item(i);
               line.addImage(item.region).size(8*2).padLeft(5f);
               line.label(()-> {
                   Tile core = state.getCore();
                   if (core == null) {
                       return "0";
                   } else {
                       return ui.formatAmount(core.entity.items.get(item));
                   }
               }).padLeft(2f);
               total ++;
               if (total % 7 == 0) {
           ...
           int nLiquid = content.liquids().size;
           for(int i=0; i<nLiquid; i++) {
               Liquid liquid = content.liquid(i);
               line.addImage(liquid.iconRegion).size(8*2).padLeft(5f);
               line.label(() -> {
                   Tile core = state.getCore();
                   if (core == null) {
                       return "0";
                   } else {
                       return ui.formatAmount((int) core.entity.liquids.get(liquid));
                   }
               }).padLeft(2f);
           }
            // ScrollPane pane = new ScrollPane(line);
            t.add(line).height(isize * 1.8f).growX();
        });
    ...
    private void addPlayButton(Table table){
        table.right().addImageButton("icon-play", "right", 30f, () -> {
            ...
            vis = ((Net.server() || players[0].isAdmin) || !Net.active());
        ...
        }).visible(() -> state.wavetime > 0 && !state.mode.disableWaves && ((Net.server() || players[0].isAdmin) || !Net.active()));
    }

    public Element getTaskInfoElement() {
        return wavetable;
    }
}
