...
public class MenuFragment extends Fragment{
    ...
    public void build(Group parent){
        ...
        //info icon
        if(mobile){
            parent.fill(c -> c.top().left().addButton("", "info", ui.about::showCredits).size(84, 45)
                    .visible(() -> state.is(State.menu)));
        }

        //version info
        parent.fill(c -> c.bottom().left().add(Strings.formatArgs("荒野之王 v{0} {1} {2} {3}", Version.number, Version.modifier, ("official".equals(Version.type) ? "":Version.type), (Version.build == -1 ? "custom build" : "build " + Version.build)))
                .visible(() -> state.is(State.menu)));
    }
    ...
    private void buildMobile(){
        ...
        MobileButton
            ...
            guide = new MobileButton("icon-tutorial", isize, "$text.tutorial", Vars.guide::forceLaunch);
    ...
    private void showPlaySelect(){
        if (Vars.guide.checkLaunch()) return;
        float w = 220f;
        float bw = w * 2f + 10f;
