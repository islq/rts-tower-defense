...
public class PlacementFragment extends Fragment{
    ...
    public void build(Group parent){
        parent.fill(full -> {
            toggler = full;
            full.bottom().right().visible(() -> !state.is(State.menu) && !Vars.guide.noPlaceMenu());
            ...
            full.table(frame -> {
                ...
                Runnable rebuildCategory = () -> {
                    ...
                    for(Recipe recipe : Recipe.getByCategory(currentCategory)){
                        ...
                        recipeElementMap.put(recipe, button);
                ...
                frame.table("button-edge-2", top -> {
                        ...
                        if(lastDisplay != null){ //show selected recipe
                            topTable.table(header -> {
                                ...
                                if(control.unlocks.isUnlocked(Recipe.getByResult(lastDisplay))){
                                    helpIconElement = header.addButton("?", "clear-partial", () -> ui.content.show(Recipe.getByResult(lastDisplay)))
                                        .size(8 * 5).padTop(-5).padRight(-5).right().grow().getElement();
                                }
                            }).growX().left();
                            topTable.row();
                    ...
                    top.row();
                    top.labelWrap(() -> Bundles.get("text.category.name." + currentCategory.toString())).top().left().width(190f).padLeft(5);
                    // top.row();
                    // top.labelWrap(() -> Bundles.get("text.category.desc." + currentCategory.toString())).top().left().width(190f).padLeft(5);
                    top.row();
                    top.addImage("blank").growX().color(Palette.accent).height(3f);
                }).colspan(3).fillX().visible(() -> getSelected() != null || tileDisplayBlock() != null || currentCategory != null).touchable(Touchable.enabled);
                frame.row();
                ...
                frame.table(categories -> {
                    ...
                    for(Category cat : Category.values()){
                        ...
                        categoryElementMap.put(cat, categories.addImageButton("icon-" + cat.name(), "clear-toggle",  16*2, () -> {
                            currentCategory = cat;
                            rebuildCategory.run();
                        }).checked(currentCategory == cat).group(group).getElement());

                        if(cat.ordinal() %2 == 1) categories.row();
                    }

                    categories.addImage("flat").grow();
    ...
    /**Show or clear the placement menu.*/
    ...
    HashMap<Category, ImageButton> categoryElementMap = new HashMap<Category, ImageButton>();
    HashMap<Recipe, ImageButton> recipeElementMap = new HashMap<Recipe, ImageButton>();
    Element helpIconElement = null;
    public Element getCategoryIcon(Category production) {
        return categoryElementMap.get(production);
    }
    public Element getRecipIcon(Recipe recipe) {
        return recipeElementMap.get(recipe);
    }
    ...
    public Element getHelpIcon() {
        return helpIconElement;
    }
}