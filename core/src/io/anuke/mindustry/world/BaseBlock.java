...
public abstract class BaseBlock extends MappableContent{
    ...
    public void tryDumpLiquid(Tile tile, Liquid liquid){
        ...
        if (core != null) {
            float flow = tile.entity.liquids.get(liquid);
            if(core.block().acceptLiquid(core, tile, liquid, flow)){
                core.block().handleLiquid(core, tile, liquid, flow);
...
//        }
    ...
    public void offloadNear(Tile tile, Item item){
        Tile core = Vars.state.getTeamCore(tile.getTeam());
        if (core != null) {
            if (core.block().acceptItem(item, core, tile) && canDump(tile, core, item)) {
                core.block().handleItem(item, core, tile);
                return;
            }
        }
//
