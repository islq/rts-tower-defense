...
import static io.anuke.mindustry.content.Liquids.oil;
...
public class NuclearReactor extends PowerGenerator{
    ...
    public void setStats(){
        super.setStats();
        stats.add(BlockStat.inputLiquid, new LiquidFilterValue(liquid -> liquid.temperature <= 0.5f && liquid.flammability < 0.1f));
    ...
    public void update(Tile tile){
        ...
        if (liquidAmount[0] > 0.1) {
            core.entity.liquids.forEach2((liquid, amount) -> {
                if(liquid.temperature > 0.5f || liquid.flammability > 0.1f || liquidAmount[0] < 0.1) return;
        ...
        if (entity.items.total() < itemCapacity) {
